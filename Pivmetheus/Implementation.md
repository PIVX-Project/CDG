...
The <sup>2</sup> voting layer is the most important voting layer, as it clearly actually represents two dimensions according to the exponent.  Simultaneously, the <sup>2</sup> layer is also the most difficult layer to successfully capture because capturing it means getting vote data from individuals who are small investors, who are only minimally to moderately interested in the outcome of the system. Besides potential predation from the other layers, voter apathy threatens to weaken the effectiveness of the <sup>2</sup> voting layer.  Besides the information timing section above. NOTE without adequate protection of the <sup>2</sup> vote, the model collapses. some options for strengthening this layer are as described below.  Some of these options are more popular than others, so they are listed in an order of reasonable implementation priority. The idea is to implement them starting at the top, and if more <sup>2</sup> protection seems to be required, then we implement further down the list. Number six is far down the list because it is both unpopular and difficult to implement.  However, early voting count results could show it to be necessary. :

1. Power voting as described above, and exponents of 0.6 and 1.66 for <sup>2</sup> and I layers respectively.
...
7. Allowing exchanges to vote explicitly and only in the <sup>2</sup> layer would bolster the stability of the layer without breaking the <sup>2</sup>SI model. Many have objected to this idea. It's possible in attempt to appease such to reduce the vote weight of the exchanges.
8. Any other accounts or classes of accounts that can be positively identified as <sup>2</sup> accounts could be factored into <sup>2</sup> in order to bolster the meaning and stability of the layer. An interesting option for implementing this would be to offer additional <sup>2</sup> vote to public keys that show large amounts of trading.  Accomplishing this would require some careful management of network calculation math. An adequate mathematics solution could potentially raise the priority of this option.
...
2. Create the <sup>2</sup> and I layers and options 1 and 2 from section 3.2 as well as a minimal set of measurement instruments for observing the system. Most especially, least squared exponential and power law approximations as well as vote statistics.
